    ...
    if (value is Map) {
      ...
      assert(() {
        assert(json.containsKey(r'cooldown'),
            'Required key "GEOrderTransactionSchema[cooldown]" is missing from JSON.');
        assert(json[r'cooldown'] != null,
            'Required key "GEOrderTransactionSchema[cooldown]" has a null value in JSON.');
        assert(json.containsKey(r'order'),
            'Required key "GEOrderTransactionSchema[order]" is missing from JSON.');
        assert(json[r'order'] != null,
            'Required key "GEOrderTransactionSchema[order]" has a null value in JSON.');
        assert(json.containsKey(r'character'),
            'Required key "GEOrderTransactionSchema[character]" is missing from JSON.');
        assert(json[r'character'] != null,
            'Required key "GEOrderTransactionSchema[character]" has a null value in JSON.');
        return true;
      }());
