...
class MyAccountDetailsSchema {
  /// Returns a new [MyAccountDetailsSchema] instance.
  ...
  /// Returns a new [MyAccountDetailsSchema] instance and imports its values from
  /// [value] if it's a [Map], null otherwise.
  // ignore: prefer_constructors_over_static_methods
  static MyAccountDetailsSchema? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        assert(json.containsKey(r'data'),
            'Required key "MyAccountDetailsSchema[data]" is missing from JSON.');
        assert(json[r'data'] != null,
            'Required key "MyAccountDetailsSchema[data]" has a null value in JSON.');
        return true;
      }());

      return MyAccountDetailsSchema(
        data: MyAccountDetails.fromJson(json[r'data'])!,
      );
    }
    return null;
  }
  ...
  static List<MyAccountDetailsSchema> listFromJson(
    ...
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = MyAccountDetailsSchema.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, MyAccountDetailsSchema> mapFromJson(dynamic json) {
    final map = <String, MyAccountDetailsSchema>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = MyAccountDetailsSchema.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
