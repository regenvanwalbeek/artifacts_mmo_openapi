...
class SkillResponseSchema {
  ...
  @override
  bool operator ==(Object other) => identical(this, other) || other is SkillResponseSchema &&
    other.data == data;
    ...
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "SkillResponseSchema[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "SkillResponseSchema[$key]" has a null value in JSON.');
        });
        return true;
      }());
  ...
  static List<SkillResponseSchema> listFromJson(dynamic json, {bool growable = false,}) {
  ...
  static Map<String, List<SkillResponseSchema>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<SkillResponseSchema>>{};
    if (json is Map && json.isNotEmpty) {
      // ignore: parameter_assignments
      json = json.cast<String, dynamic>();
      for (final entry in json.entries) {
        map[entry.key] = SkillResponseSchema.listFromJson(entry.value, growable: growable,);
      }
    }
    return map;
  }
