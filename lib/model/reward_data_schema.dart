    ...
    if (value is Map) {
      ...
      assert(() {
        assert(json.containsKey(r'cooldown'),
            'Required key "RewardDataSchema[cooldown]" is missing from JSON.');
        assert(json[r'cooldown'] != null,
            'Required key "RewardDataSchema[cooldown]" has a null value in JSON.');
        assert(json.containsKey(r'rewards'),
            'Required key "RewardDataSchema[rewards]" is missing from JSON.');
        assert(json[r'rewards'] != null,
            'Required key "RewardDataSchema[rewards]" has a null value in JSON.');
        assert(json.containsKey(r'character'),
            'Required key "RewardDataSchema[character]" is missing from JSON.');
        assert(json[r'character'] != null,
            'Required key "RewardDataSchema[character]" has a null value in JSON.');
        return true;
      }());
