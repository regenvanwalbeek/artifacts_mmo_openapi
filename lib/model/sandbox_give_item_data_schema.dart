    ...
    if (value is Map) {
      ...
      assert(() {
        assert(json.containsKey(r'cooldown'),
            'Required key "SandboxGiveItemDataSchema[cooldown]" is missing from JSON.');
        assert(json[r'cooldown'] != null,
            'Required key "SandboxGiveItemDataSchema[cooldown]" has a null value in JSON.');
        assert(json.containsKey(r'item'),
            'Required key "SandboxGiveItemDataSchema[item]" is missing from JSON.');
        assert(json[r'item'] != null,
            'Required key "SandboxGiveItemDataSchema[item]" has a null value in JSON.');
        assert(json.containsKey(r'character'),
            'Required key "SandboxGiveItemDataSchema[character]" is missing from JSON.');
        assert(json[r'character'] != null,
            'Required key "SandboxGiveItemDataSchema[character]" has a null value in JSON.');
        return true;
      }());
