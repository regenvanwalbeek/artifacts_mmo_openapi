...
class BankGoldTransactionResponseSchema {
  /// Returns a new [BankGoldTransactionResponseSchema] instance.
  ...
  /// Returns a new [BankGoldTransactionResponseSchema] instance and imports its values from
  /// [value] if it's a [Map], null otherwise.
  // ignore: prefer_constructors_over_static_methods
  static BankGoldTransactionResponseSchema? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        assert(json.containsKey(r'data'),
            'Required key "BankGoldTransactionResponseSchema[data]" is missing from JSON.');
        assert(json[r'data'] != null,
            'Required key "BankGoldTransactionResponseSchema[data]" has a null value in JSON.');
        return true;
      }());

      return BankGoldTransactionResponseSchema(
        data: BankGoldTransactionSchema.fromJson(json[r'data'])!,
      );
    }
    return null;
  }
  ...
  static List<BankGoldTransactionResponseSchema> listFromJson(
    ...
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = BankGoldTransactionResponseSchema.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, BankGoldTransactionResponseSchema> mapFromJson(
    ...
    if (json is Map && json.isNotEmpty) {
      ...
      for (final entry in json.entries) {
        final value = BankGoldTransactionResponseSchema.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
