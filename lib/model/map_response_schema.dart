...
class MapResponseSchema {
  ...
  @override
  bool operator ==(Object other) => identical(this, other) || other is MapResponseSchema &&
    other.data == data;
    ...
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "MapResponseSchema[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "MapResponseSchema[$key]" has a null value in JSON.');
        });
        return true;
      }());
  ...
  static List<MapResponseSchema> listFromJson(dynamic json, {bool growable = false,}) {
  ...
  static Map<String, List<MapResponseSchema>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<MapResponseSchema>>{};
    if (json is Map && json.isNotEmpty) {
      // ignore: parameter_assignments
      json = json.cast<String, dynamic>();
      for (final entry in json.entries) {
        map[entry.key] = MapResponseSchema.listFromJson(entry.value, growable: growable,);
      }
    }
    return map;
  }
